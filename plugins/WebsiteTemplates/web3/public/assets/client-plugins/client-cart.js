...
document.addEventListener("DOMContentLoaded", function(event){
  ...
  else if(user_details != null)
  {
    $.ajax({
      type : 'GET',
      url : callApiUrl+'?user_id='+user_id+'&type=2&website_id='+project_settings.websiteid,
      dataType : 'json',
...
//  console.log(carAmountHtml);
  ...
  axios({
      method: 'GET',
      url : project_settings.shopping_api_url+'?user_id='+user_id+'&type=2&website_id='+project_settings.websiteid,
    })
        ...
        for (var key in response_data) {
          ...
          let productData = await getProductDetailById(response_data[key].product_id)
            ...
            //   success:  function (data) {
                ...
                let detailLink = project_settings.base_url+'public/productdetail.html?locale='+project_settings.default_culture+'&pid='+response_data[key].product_id;
                ...
                if(typeof productData.features != 'undefined')
                {
                	for (let i in productData.features)
                    {
                		if(typeof productData.features[i].key != 'undefined')
                        {
                    		if(productData.features[i].key.toLowerCase() == 'size')
                			{
                        		var listHtmlReplace = listHtmlReplace.replace(/#data.size#/g,productData.features[i].value);
                			}
                        }
                    }
                }
...
//alert(imprint_info.imprint_method_name)
                ...
                // Shipping Section
                if(typeof response_data[key].shipping_method != "undefined" && false)
                {
