...
Y({
  ...
  y.share.wishList.observe(function (event) {
      if (event.type === 'insert') {
        ...
        if(window.yList.share.wishList._content != null && window.yList.share.wishList._content.length == 0)
        {
          ...
          $("span#wishlistCount").html(0);
        }
        else{
          // document.getElementById("wishlistCount").innerHTML =  window.yList.share.wishList._content.length;
          $("span#wishlistCount").html(window.yList.share.wishList._content.length);
        }
        // showWishList();
      }
  })
  ...
  $(document).ready(function() {
    ...
    if(user_id != null) {
      ...
      $(".js_after_login").addClass("hide")
      $(".js_before_login").removeClass("hide")
    }else{
      $(".js_after_login").removeClass("hide")
      $(".js_before_login").addClass("hide")
    }
...
$(document).on('click', '.js-remove-wishlist', function(e) {
  ...
  bootbox.confirm("<h3 class='text-center'>Are you sure want to delete?</h3>", function(result)
    {
      if(result)
        {
            showPageAjaxLoading();
...
function updateShoppingDatabaseCount(type, operation) {
  if (type == 1) {
    let wishCount = eval(parseInt($("span#wishlistCount").html())+operation+1);
    // document.getElementById("wishlistCount").innerHTML = Math.max(0, wishCount);
    $("span#wishlistCount").html(Math.max(0, wishCount));
  }
  if(type == 2){
...
function showErrorMessage(error_message) {
  error_message = error_message.charAt(0).toUpperCase() + error_message.slice(1).toLowerCase();
    ...
    $('body').prepend('<div class="container"><div class="alert alert-danger" style="margin-top:-50px;z-index: 9999992;"><a data-dismiss="alert" class="close" href="javascript:void(0)">&times;</a><span>'+ error_message+'</span></div><div class="clr"></div></div>');
  ...
  success_message = success_message.charAt(0).toUpperCase() + success_message.slice(1).toLowerCase();
	...
	if($('.alert-success').length) {
		...
		if( $('#modal-table').is(":visible") && $('#modal-table .modal-content').length>0 ){
			$('body').prepend('<div class="container"><div class="alert alert-success" style="margin-top:-50px;z-index: 9999992;"><a data-dismiss="alert" class="close" type="button">&times;</a> <span>'+ success_message+'</span></div><div class="clr"></div></div>');
		} else {
			$('body').prepend('<div class="container"><div class="alert alert-success" style="margin-top:-50px;z-index: 9999992;"><a data-dismiss="alert" class="close" type="button">&times;</a> <span>'+ success_message+'</span></div><div class="clr"></div></div>');
		}
...
function showWishList(recetAdded=false)
{
  ...
  if($("#myWishList").length > 0)
  {
    ...
    if(wishlistValuesCount)
    {
      ...
      $("span#wishlistCount").html(wishlistValuesCount);
    }
    else{
      // document.getElementById("wishlistCount").innerHTML = 0;
      $("span#wishlistCount").html(0);
