...
function deleteFromLocal(type,product_id){
  ...
  if(type == 3) {
    ...
    else
    {
      ...
      {
        $('#myCompareList #listing .ob-product-compare .compare-block').css('width',215*window.yList.share.compareList._content.length+'px')
      }
      else{
        $('#myCompareList #listing .ob-product-compare .compare-block').css('width',215*5+'px')
      }
      // window.yList.share.compareList.delete(0)
...
function updateShoppingLocalCount(count , type) {
  if (type == 1) {
    ...
    document.getElementById("wishlistCount1").innerHTML = count;
  }
  if(type == 2){
...
function updateShoppingDatabaseCount(type, operation) {
  if (type == 1) {
    ...
    document.getElementById("wishlistCount1").innerHTML = eval(parseInt(document.getElementById("wishlistCount").innerHTML)+operation+1);
  }
  if(type == 2){
...
function showWishList(recetAdded=false)
{
  ...
  var poplistHtml = $('#popupmywishlist .popuplisting').html();
  ...
  {
    if (typeof(listHtml) !== "undefined" && wishlist_values != "" ) {finalhtml='';
      for (item in wishlist_values)
            ...
            if(wishlist_values[item] != null)
            {
              $.ajax({
                ...
                success: function (data) {
                  ...
                  let poplistHtml1 = poplistHtml.replace('#data.image#',project_settings.product_api_image_url+productData[0]._source.default_image);
                  poplistHtml1 = poplistHtml1.replace(/#data.id#/g,wishlist_values[item].val.id);
                  poplistHtml1 = poplistHtml1.replace('#data.title#',productData[0]._source.product_name);
                  poplistHtml1 = poplistHtml1.replace('#data.sku#',productData[0]._source.sku);
                  poplistHtml1 = poplistHtml1.replace('#data.price#',productData[0]._source.price_1);
                  poplistHtml1 = poplistHtml1.replace('#data.currency#',productData[0]._source.currency);
        ...
        $('#popupmywishlist .popuplisting1').html(finalhtml);
    } else {
    ...
    if(wishlistValuesCount)
    {
      ...
      document.getElementById("wishlistCount1").innerHTML= wishlistValuesCount;
    }
    else{
    ...
    {
      document.getElementById("wishlistCount").innerHTML = wishlistValuesCount;
      if(document.getElementById("wishlistCount1"))
      {
        document.getElementById("wishlistCount1").innerHTML = wishlistValuesCount
      }
    }
...
function showCompareList(recetAdded=false)
{
  ...
  var poplistHtml = $('#popupmycompare .popuplisting').html();
  ...
  if($("#myCompareList").length > 0)
  {
    ...
    if (typeof(compareHtml.html()) !== "undefined" && compare_values != null && compare_values.length > 0) {finalhtml='';
      ...
      //   }
            ...
            if(showItem)
            {
              ...
              $.ajax({
                ...
                success: function (data)
                {
                  ...
                  let poplistHtml1 = poplistHtml.replace('#data.image#',project_settings.product_api_image_url+productData[0]._source.default_image);
                  poplistHtml1 = poplistHtml1.replace(/#data.id#/g,compare_values[item].val.id);
                  poplistHtml1 = poplistHtml1.replace('#data.title#',productData[0]._source.product_name);
                  poplistHtml1 = poplistHtml1.replace('#data.sku#',productData[0]._source.sku);
                  poplistHtml1 = poplistHtml1.replace('#data.price#',productData[0]._source.price_1);
                  poplistHtml1 = poplistHtml1.replace('#data.currency#',productData[0]._source.currency);
                  ...
                  if(recetAdded)
                  {
                    ...
                    if(JSON.parse(localStorage.getItem("savedCompared")).length == 1)
                    {
                      ...
                      compareHtml.find("#item_title_price1").html("<td class='feature-block'><div class='img-add-product-compare'><p>Add Product</p><img src='images/add-compare.png'/></div></td>"+productHtml)
                      compareHtml.find("#item_sku1").html("<td class='feature-block check-quantity'>SKU</td>"+itemSkuHtml)
                      compareHtml.find("#item_summary1").html("<td class='feature-block check-quantity'>Product Detail<br/><p>You can see more information</br>about product</p></td>"+activeSummaryHtml)
          ...
          $('#popupmycompare .popuplisting1').html(finalhtml);
    } else {
      // $("#myCompareList #listing div:first").addClass("hide");
    ...
    {
      $('#myCompareList #listing .ob-product-compare .compare-block').css('width',215*compareValuesCount+'px')
    }
    else{
      $('#myCompareList #listing .ob-product-compare .compare-block').css('width',215*5+'px')
    }
          ...
          if(user_details != null){
            ...
            success: function (data)
            {
              if(data != '' || data != null)
              {
                ...
                if(typeof productData[0] != 'undefined')
                {
                  let poplistHtml1 = poplistHtml.replace('#data.image#',project_settings.product_api_image_url+productData[0]._source.default_image);
                  poplistHtml1 = poplistHtml1.replace(/#data.id#/g,compare_values[item].val.id);
                  poplistHtml1 = poplistHtml1.replace('#data.title#',productData[0]._source.product_name);
